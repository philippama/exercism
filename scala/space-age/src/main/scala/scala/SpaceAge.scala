...
object SpaceAge {
    ...
    def onEarth(ageInSeconds: Double): Double = {
        toPlanetYears(ageInSeconds, earthYearSeconds)
    }

    def onMercury(ageInSeconds: Double): Double = {
        toPlanetYears(ageInSeconds, mercuryYearSeconds)
    }

    def onVenus(ageInSeconds: Double): Double = {
        toPlanetYears(ageInSeconds, venusYearSeconds)
    }

    def onMars(ageInSeconds: Double): Double = {
        toPlanetYears(ageInSeconds, marsYearSeconds)
    }

    def onJupiter(ageInSeconds: Double): Double = {
        toPlanetYears(ageInSeconds, jupiterYearSeconds)
    }

    def onSaturn(ageInSeconds: Double): Double = {
        toPlanetYears(ageInSeconds, saturnYearSeconds)
    }

    def onUranus(ageInSeconds: Double): Double = {
        toPlanetYears(ageInSeconds, uranusYearSeconds)
    }

    def onNeptune(ageInSeconds: Double): Double = {
        toPlanetYears(ageInSeconds, neptuneYearSeconds)
    }
